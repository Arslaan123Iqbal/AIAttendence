...
class Video2(object):
    def __init__(self):
        self.video= cv2.VideoCapture("http://192.168.18.3:8080/video")
    ...
    def get_frame(self):
        ...
        for(x,y,w,h) in faces:
            ...
            if(profile!=None):
                ...
                cv2.rectangle(img,(x,y-35),(x,y),(0,255,0),cv2.FILLED)
                ...
                # data_tuple = ()
                cur = conn.cursor()
                cur.execute('SELECT * FROM attendence_out WHERE (id=? AND date=?)', (profile[0], today))
                entry = cur.fetchone()

                if entry is None:
                    conn.execute( """INSERT INTO attendence_out
                            ( name,register,department, date, time,status) 
                            VALUES (?, ?,?, ?,?,?) """, (profile[1], profile[2],profile[4] ,today, current_time,"Out"))
                    print("PResent")
                else:
                    print("ENtry found")
